...
def calcular_ano_ultrapassagem_populacional(
    ...
    mensagem = f"'A taxa de crescimento do país B ({(taxa_crescimento_populacao_maior * 100):.1f}%) deve ser menor do que a do país A ({(taxa_crescimento_populacao_menor * 100):.1f}%)'"
    if taxa_crescimento_populacao_menor > taxa_crescimento_populacao_maior:
        ...
        while populacao_a <= populacao_b:
            ano += 1
            populacao_a += (populacao_a * taxa_crescimento_populacao_menor)
            populacao_b += (populacao_b * taxa_crescimento_populacao_maior)
        
        mensagem = (f"'População de A, depois de {ano} ano(s) será de {populacao_a:.0f} pessoas, superando a de B, que será de {populacao_b:.0f} pessoas'")    
    print(mensagem)
